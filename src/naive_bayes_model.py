...
# Train Naive Bayes (GaussianNB): classification_report + Confusion Matrix + Nhận xét Recall
...
def _print_recall_comment(report_dict, class_names):
    ...
    print("\n  --- Nhan xet Recall theo tung loai tan cong (Naive Bayes) ---")
...
def train_naive_bayes(X_train, y_train, X_test, y_test, le):
    print("=" * 60)
    ...
    plt.title("Confusion Matrix - Naive Bayes (GaussianNB)", fontsize=14, fontweight="bold")
    ...
    joblib.dump(nb_model, os.path.join(MODELS_DIR, "naive_bayes.pkl"))
    print("  [OK] Da luu: models/naive_bayes.pkl")
